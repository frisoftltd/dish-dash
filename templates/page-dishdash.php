 ...
 *
 * v3.18.16: Reviews fetch/pooling/debug logic extracted out of this file
 * into DD_Homepage_Module::get_reviews_with_debug() (shared with Minimal
 * Light) — this file now just calls it. No behavior change.
 *
...
<!-- ══ REVIEWS ═════════════════════════════════════════════════════════════ -->
<?php if ( $reviews_vis ) :
    // Fetch + debug now come from the shared pipeline (extracted to
    // DD_Homepage_Module::get_reviews_with_debug() in v3.18.16) — this
    // template originally built and proved out that logic; Minimal Light
    // now calls the exact same method. No behavior change here.
    $dd_review_result = class_exists( 'DD_Homepage_Module' ) ? DD_Homepage_Module::get_reviews_with_debug() : [ 'items' => [], 'debug' => [] ];
    $dd_review_items  = $dd_review_result['items'];
    $dd_review_debug  = $dd_review_result['debug'];
