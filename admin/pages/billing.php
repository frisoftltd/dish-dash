...
// ─────────────────────────────────────────────────────────────────────────
//  INVOICE GENERATION (v3.18.0)
//  Admin-only (Fri Soft platform staff) — never dd_restaurant_owner/
//  dd_restaurant_manager, see dd_is_platform_admin() in class-dd-helpers.php.
//  A document view/download, not a state change, so this is a plain
//  nonce-protected GET link (same "?export=csv" shape already used by the
//  Customers admin page's CSV export), not a new AJAX endpoint.
// ─────────────────────────────────────────────────────────────────────────
...
if ( isset( $_GET['dd_invoice'] ) || isset( $_GET['dd_invoice_pdf'] ) ) {
    $dd_inv_month = sanitize_text_field( wp_unslash( $_GET['dd_invoice'] ?? $_GET['dd_invoice_pdf'] ?? '' ) );
    if ( ! preg_match( '/^\d{4}-\d{2}$/', $dd_inv_month ) ) {
        wp_die( esc_html__( 'Invalid month.', 'dish-dash' ) );
    }
    $dd_inv_nonce_ok = wp_verify_nonce(
        sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ?? '' ) ),
        'dd_invoice_' . $dd_inv_month
    );
    if ( ! dd_is_platform_admin() || ! $dd_inv_nonce_ok ) {
        wp_die( esc_html__( 'Access denied.', 'dish-dash' ) );
    }

    $dd_inv_data = dd_invoice_get_data( $dd_inv_month );

    if ( isset( $_GET['dd_invoice_pdf'] ) ) {
        dd_invoice_stream_pdf( $dd_inv_data );
    } else {
        dd_invoice_render_page( $dd_inv_data );
    }
    exit;
}
