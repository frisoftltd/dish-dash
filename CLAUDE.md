...
>
> Last updated: v3.18.6 (2026-08-07)
...
| **Last working state** | v3.18.6 — Minimal Light restructured with a genuinely distinct layout (v3.18.5 shipped correct data-wiring but Khana Khazana's own structure — same header bar, same section shapes — just different tokens; this release replaces the structure). All v3.18.5 data-fetching reused verbatim (hero fields, category/featured/selcat/reviews queries, real-sales-data "Most Popular" sort). Structural changes: (1) **own header markup** — `page-home.php` no longer calls `wp_body_open()` (which fires the shared `inject_global_header()`, a header with a visible nav bar/search/login-signup this design can't have); renders its own icon-driven header (logo, location-pin → Google Maps link, cart, hamburger) instead, deliberately reusing the shared header's exact IDs/classes (`#ddMenuToggle`, `#ddNavDrawer`, `#ddDrawerOverlay`, `#ddCartTopBtn`, `#ddCartCount`) so existing `frontend.js`/`cart.js` handlers and `theme.css`'s drawer-slide animation keep working with zero JS changes — `wp_footer()` still called unchanged, so footer/cart-drawer/bottom-nav/reservation-modal/product-modal are all still the real shared chrome, restyled via CSS only. Khana Khazana's own header is completely unaffected (still calls `wp_body_open()` as before). (2) Split-screen hero; all 3 configured CTAs render as understated text-links (confirmed with developer, not a silent 2-of-3 drop). (3) New Story section right after hero — no dedicated Settings field exists for this, so it reuses `dd_footer_description` (text) + the hero-image fallback chain (photo), respecting the existing "Show Footer Description" toggle. (4) Category index is a bordered CSS-grid table, not circles. (5) Featured/Selected-Category/Reviews are horizontal-scroll strips (Featured/Reviews already were in v3.18.5; Selected Category's heading now updates live via the existing tab-click JS to show "Chef's Selection — {active category}"). (6) Reserve is a full-width dark band with an understated "Book now →" link, same `#dd-open-reservation` trigger. Default heading copy changed (still fully admin-overridable): "The Menu", "From the Kitchen", "Chef's Selection", "What People Say". One deliberate deviation, reported not silently done: the mobile bottom nav's 4th icon stays Cart (not Profile per the mockup) — that nav is rendered by the same function that owns the cart drawer as its "single source of truth" (`inject_cart_sidebar()`), and forking it to swap one icon would mean duplicating shared cart markup the guardrails explicitly protect; restyled only. Full per-version history: see RELEASE.md. |
