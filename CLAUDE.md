...
>
> Last updated: v3.18.3 (2026-08-06)
...
| **Last working state** | v3.18.3 — Audit hidden from Owner/Manager sidebar + submenu reordered. Audit (`dish-dash-audit`, capability `manage_options`) was never added to the existing Owner/Manager CSS-hide block (`dishdash-core/class-dd-hooks.php`) that already hides Settings/Tools/Template/Activity Log — an omission, not a deliberate gate; added it there, same pattern, `remove_submenu_page()` still banned (v3.5.13 regression, enforced by the Audit module's own scanner). Submenu order for Owner/Manager is now Dashboard, Orders, Reservations, Customers, Analytics, Billing, Brand Identity, Homepage, Auth & Login. **Mechanism note**: hook-priority reordering (the existing Activity Log precedent) can't achieve this — Orders and Analytics are both emitted inside one `DD_Admin::register_admin_menus()` callback, so nothing can interleave between them via priority alone without editing that file (out of scope). Used the WP-native `$position` argument on `add_submenu_page()` instead, only on the two calls that needed to move — `DD_Customers_Module::register_admin_page()` and `DD_Reservations_Admin::register_submenu()`, both `position=2` (right after Orders, before the always-present-but-hidden Menu Items/Delivery/Branches/POS block). Since `DD_Loader` instantiates Customers before Reservations, Reservations' later splice at the same index lands to Customers' left, producing the exact target order. Tables/Seating Sections were left unpositioned (not named in the brief) and now trail at the very end of the visible list. No capability/role logic touched beyond the one Audit CSS addition. Full per-version history: see RELEASE.md. |
