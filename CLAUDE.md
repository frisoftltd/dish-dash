...
>
> Last updated: v3.9.6 (2026-06-19)
...
| Field | Value |
|---|---|
| **Deployed version** | v3.9.6 |
| **Current phase** | Phase 7 — Role Cleanup & Access Control |
| **Current sub-phase** | Phase 7B — COMPLETE |
| **Next task** | v3.9.7 — Phase 7C (TBD) |
| **Last working state** | v3.9.6: Notification badge count now reflects live restaurant state (pending orders + pending reservations, restaurant-wide) returned as pending_count from ajax_poll_notifications — identical for every staff member. Badge is set authoritatively from payload.pending_count on every poll response, before the silent-poll early return. Per-browser localStorage count accumulation removed (dd_unread_count no longer drives the badge). Page-load restore no longer sets stale count from localStorage. "Mark all read" clears the panel list only; badge re-reflects live pending count on next poll. |
