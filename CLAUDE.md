...
>
> Last updated: v3.18.23 (2026-08-07)
...
| **Last working state** | v3.18.23 — Restyled the existing 3-screen mobile Menu page (Screens 1-3) for Minimal Light. Investigation (`investigation-mobile-menu.md`) confirmed the "3-screen mobile app" (`.dd-mobile-app` in `grid.php`, driven by the `DDMobileMenu` class in `menu-page.js`) already exists, is fully functional, and is universal across both templates today (zero Minimal Light styling) — and, critically, that it does **not** hand off to the shared product modal on mobile (Screen 3 is its own independent single-product view with its own stepper/attributes/`dd_cart_add` call; the shared `#ddProductModal` is only reached from a `window.innerWidth >= 1025` desktop-viewport branch, never true mobile). This release restyles all three screens in place: Screen 1's category list converts from vertical rows to a circular 4-column icon grid (`DD_Template_Module::active_template()`-branched in `grid.php`, since the row markup never had a thumbnail/initial-letter fallback to restyle via CSS alone — Khana Khazana's row markup is untouched, byte-for-byte, in the `else` branch), plus a new "All Dishes" 2-column grid below it and a category-name/dish-count header line on Screen 2 — both Minimal-Light-only additions (don't exist in Khana Khazana's DOM), populated in `menu-page.js` via small guarded additions (`renderAllDishesGrid()`, a small append to `loadProductsForCategory()`) that call only pre-existing, unmodified methods (`renderProductList()`, `showProductDetails()`, `addToCartById()`) — no existing DDMobileMenu behavior changed. Screens 2-3's palette/typography restyle needed zero JS changes at all — unlike the v3.18.22 modal reskin, `menu-page.js` builds all its HTML via CSS classes only, no inline styles, so a pure `body.dd-tpl-minimal-light` CSS block in `menu-page.css` covers it. Category data source: standardized the new Screen 1 grid on the existing `$categories` variable (already in scope from the shortcode handler, already used by the row markup it replaces) rather than introducing a dependency on either of the other two pre-existing, slightly-divergent category queries (`$dd_menu_cats`, desktop-only; `DD_API::get_categories()`, JS-only) — no new query, no consolidation of those two (flagged as a separate follow-up). Search bar confirmed already real (`filterCategories()`/`searchProducts()`, pre-existing) — restyled only, not rebuilt. Full per-version history: see RELEASE.md. |
