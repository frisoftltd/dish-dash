...
>
> Last updated: v3.18.24 (2026-08-07)
...
| **Last working state** | v3.18.24 — Fixed the v3.18.23 category grid's right-edge overflow and unified it with the homepage's mobile Food Category List. Investigation (`investigation-category-grid-overflow.md`) found two concrete causes: (1) `grid-template-columns: repeat(4, 1fr)` on `.dd-mobile-category-list` had no `min-width: 0` on the grid items (`.dd-mobile-category-item`) — CSS Grid items default to `min-width: auto`, which floors shrinkage at the content's intrinsic minimum width, so any category name wider than its fair 1fr share forced the row past the container edge; (2) `.dd-mobile-cat-tile__photo`'s `border: 1px solid` had no `box-sizing: border-box`, and — confirmed via grep — the Menu page's `<body>` never carries the `dd-page` class theme.css's sitewide `box-sizing: border-box` reset is scoped to (only `page-home.php`'s hand-written `<body>` tag adds it), so the circle silently rendered at 66×66px instead of 64×64px. Both ancestors (`.dd-mobile-app`, `.dd-mobile-screen`) clip overflow rather than scroll, turning this into a silent cut-off. Fix: `min-width: 0` on the grid item, `overflow-wrap`/`word-break` on the name as a safety net, `box-sizing: border-box` on the photo circle. While fixing it, moved the whole category-grid ruleset from `menu-page.css` (Menu-page-only) to `assets/css/layouts/minimal-light.css` (loads on every Minimal Light page, including the Menu page — not page-gated) and reused the exact same classes (`.dd-mobile-category-list`/`.dd-mobile-category-item`/`.dd-mobile-cat-tile__photo`/`__initial`/`__count`) for the homepage's mobile Food Category List (`page-home.php`), replacing its old `.dd-ml-food-cat-row` chevron-row markup (now-dead CSS removed). One shared stylesheet governs both places' appearance going forward; the two contexts still use different grid-item elements as their own interaction models require (`<li data-cat-id>` + `DDMobileMenu`'s JS click delegate on the Menu page; plain `<a href>` direct navigation on the homepage, no JS) — the `dd_section_food_cat_list_mobile` "Show on Mobile" toggle in Homepage Settings wraps this section's PHP untouched, so it keeps working exactly as before. Full per-version history: see RELEASE.md. |
