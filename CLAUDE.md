...
>
> Last updated: v3.17.3 (2026-08-06)
...
| Field | Value |
|---|---|
| **Deployed version** | v3.17.3 (⚠️ NOT YET VERIFIED — regression check against live totals required before treating this cutover as done, see below) |
...
| **Last working state** | v3.17.3 — Billing page cutover to the ledger. `admin/pages/billing.php`'s This Month / Last Month / All Time / Monthly History (both Orders and Reservations) now read `COUNT(*)`/`SUM(amount)` from `wp_dishdash_billing_ledger` (`WHERE source_type=..., is_test=0, billable_month=...`) instead of live `platform_fee` queries against `wp_dishdash_orders`/`wp_dishdash_reservations`. Fixes two proven instability sources: a no-deposit reservation's billable status changing after the fact (late no-show silently un-counting it on reload), and `platform_fee` itself being zeroed by a later event (e.g. disabling the flat fee), which erased historical billing truth under the old live query. Status Breakdown cards (both sections) and the Fee Per Order/Reservation rate displays deliberately stay live/unchanged — the ledger has no concept of full status distribution, and rate cards were never historical sums. The v3.17.1 "combined, frozen" workaround for Paid months is removed — now that Monthly History itself reads from the append-only ledger, each table's own ledger-sourced monthly sum is already the correctly-split, stable number, so `wp_dd_billing_payments.amount` (still written unchanged by `ajax_mark_month_paid()`, still combined-only) no longer needs to be read for display at all. **⚠️ Not verified against live data by Claude Code — no DB access.** A concrete, code-level finding surfaced during this work: the *live* no-deposit reconciliation sweep (`run_reconcile_sweep()`, unchanged in this release per the brief) stamps `billable_month` as the sweep's *current* month, not the reservation's own date — unlike the backfill script, which correctly uses the historical date. A booking near a month boundary whose grace period crosses into the next calendar month could log under the wrong month going forward. Flagged as a follow-up for `DD_Billing_Ledger_Module::run_reconcile_sweep()`, not fixed here (guardrail: do not touch v3.17.0 triggers in this release). Full per-version history: see RELEASE.md. |
